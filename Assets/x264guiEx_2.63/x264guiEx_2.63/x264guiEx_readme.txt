...
x264を分離し、別プロセスとして実行できようにしたものです。
...
x264guiExのアンインストール
http://rigaya34589.blog135.fc2.com/blog-entry-472.html
...
をみていただければよいのですが、念のためここにも書いておきます。

1.
x264guiEx_2.xx.zipを解凍してください。
...
「次へ」をクリックしてください。
すると、新たなウィンドウが開きます。
「x264guiExでは音声エンコーダとして、neroaacencを使用します。...」
...
8.
Aviutlを起動し、「その他」>「出力プラグイン情報」にx264guiEx 2.xxがあるか確かめます。
ここでx264guiExの表示がない場合、インストールに失敗したなどが考えられます。
...
【簡易インストーラについて】
auo_setup.exe
.NET Framework をインストールし、auoSetup.exeを使用可能にします。libcurlを使用しています。

setup\\auoSetup.exe
.NET を利用した簡易インストーラ本体です。zlibを使用しています。

7z.exe / 7z.dll
これらは7-zipプロジェクトの一部です。
7-ZipはGNU LGPLのライセンスであり、そのソースコードはwww.7-zip.orgから得ることができます。

libcurl, zlib, 7-zipのライセンスファイルはsetup\licence以下に同梱しています。
